...
def inittcl(args):
    ...
    current_procfile_basenames=[os.path.basename(x) for x in glob.glob(proc_dir+'/*.tcl')]
    if (not 'macros.tcl' in current_procfile_basenames) or force:
        logger.info(f'Generating macros.tcl in package directory {proc_dir}')
        p=Psfgen(c)
        p.newfile(os.path.join(proc_dir,'macros.tcl'))
        p.addline('# This file generated by pestifer initttcl')
        p.addline(f'source {proc_dir}/util.tcl')
        p.atomselect_macros()
        p.writefile()

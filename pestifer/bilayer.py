...
class BilayerEmbedTask(BaseTask):
    ...
    def do(self):
        ...
        # length_pad=self.specs.get('length_pad',3.0)
        # prot_rad_buf=self.specs.get('prot_rad_buf',2.0)
        ...
        # this is just temporary until i implement ion specs
        ncl=npot=0
        if sg=='+':
            ncl=int(global_charge)
        else:
            npot=int(np.abs(global_charge))

        ions={'CLA':{'n':ncl,'claimed':0},'POT':{'n':npot,'claimed':0}}
        ...
        if 'embed' in self.specs:
            ...
            pm.addline( '  number 1')
            pm.addline(f'  fixed {" ".join([f"{x:.3f}" for x in shift])} 0. 0. 0.')
        ...
        for leaflet in [LL,UL]:
            ...
            for specs in leaflet['lipids']:
                ...
                if length>avail_depth:
                    ...
                    pm.addline(f'   atoms {specs["below-z-atom"]}')
                    pm.addline(f'       below plane 0. 0. 1. {specs["below-z"]:.3f}')
                    pm.addline(f'   end atoms')
                    pm.addline(f'   atoms {specs["above-z-atom"]}')
                    pm.addline(f'       above plane 0. 0. 1. {specs["above-z"]:.3f}')
                    pm.addline(f'   end atoms')
                else:
                    ...
                    pm.addline(f'   inside box {box_ll[0]:.3f} {box_ll[1]:.3f} {specs["within-z-lo"]:.3f} {box_ur[0]:.3f} {box_ur[1]:.3f} {specs["within-z-hi"]:.3f}')
                    pm.addline(f'   constrain_rotation x {constrain_rotation} 10.0')
                    pm.addline(f'   constrain_rotation y {constrain_rotation} 10.0')
                pm.addline(f'    nloop {nloop}')
        ...
        if 'embed' in self.specs:
            ...
            pm.addline( '  number 1')
            pm.addline(f'  fixed 0. 0. 0. 0. 0. 0.')
            pm.addline( 'end structure')
        ...
        for chamber in [LC,UC]:
            ...
            for comp,n in components.items():
                if n>0:
                    ...
                    pm.addline(f'    number {n}')
                    pm.addline(f'    inside box {box_ll[0]:.3f} {box_ll[1]:.3f} {chamber["z-lo"]:.3f} {box_ur[0]:.3f} {box_ur[1]:.3f} {chamber["z-hi"]:.3f}')
                    pm.addline(f'    nloop {nloop}')
                    pm.addline(f'end structure')
        pm.writefile()
        self.result=pm.runscript()

...
class ExampleManager:
    ...
    def _inject_inspection_annotation(text: str, findings) -> str:
        """Insert the inspection findings (as YAML comment lines) into the dumped config, right after
        the psfgen ``source:``'s ``biological_assembly`` line."""
        lines = text.splitlines()
        out = []
        injected = False
        for line in lines:
            out.append(line)
            if not injected and line.strip().startswith('biological_assembly:'):
                out.extend(findings.annotation_lines())
                injected = True
        if not injected:      # template changed; fall back to appending
            out.extend([''] + findings.annotation_lines(indent=''))
        return '\n'.join(out) + '\n'

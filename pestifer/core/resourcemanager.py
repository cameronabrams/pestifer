...
class ResourceManager:
    ...
    def update_atomselect_macros(self):
        ...
        with open(p,'w') as f:
            f.write(self.atomselect_macros_text())

    # CGenFF's general small-molecule library: ~2,000 drug-like residues that no pestifer workflow
    # selects by name.  In a VMD macro they make every selection that uses the keyword ~400x slower
    # (measured: 225 ms against 0.6 ms on a 35,000-atom system), so they are left out.  Per-residue
    # custom CGenFF streams (<RESI>-cgenff.str) are kept.
    _MACRO_EXCLUDED_TOPFILES = frozenset({'top_all36_cgenff.rtf'})

    def atomselect_macros_text(self) -> str:
        """
        The full text of ``macros.tcl``: one VMD atomselect macro per macro segtype, listing every
        residue pestifer classifies under it as shipped -- curated and derived alike, never a
        runtime or user-custom addition -- except CGenFF's bulk library.  The file used to be generated from the curated lists only; once classification
        moved to the derived table, regenerating it would have shrunk the ``lipid`` and ``glycan``
        keywords that pestifer's own Tcl selects on.
        """
        import io
        ...
        for vd in self.charmmff_version_dirs():
            for resname, topfile in CHARMMFFContent(vd).resi_to_topfile_map.items():
                topfile_of.setdefault(resname, topfile)
        from .labels import package_classification
        by_segtype = defaultdict(set)
        for resname, segtype in package_classification().items():
            if topfile_of.get(resname) not in self._MACRO_EXCLUDED_TOPFILES:
                by_segtype[segtype].add(resname)
        f = io.StringIO()
        ...
        f.write('## Each macro lists every residue pestifer classifies under it (curated and derived), except\n')
        f.write('## the CGenFF general library top_all36_cgenff.rtf, which would slow selections ~400x.\n\n')
        self.labels.update_atomselect_macros(f, resnames_by_segtype=by_segtype)
        return f.getvalue()
    ...
    def regenerate_derived_segtypes(self, out_path=None) -> dict:
        ...
        linked: set[str] = set()
        heavy_atoms: dict[str, list[str]] = {}
        for vd in self.charmmff_version_dirs():
            content = CHARMMFFContent(vd)
            for resname, topfile in content.resi_to_topfile_map.items():
                merged.setdefault(resname, topfile)
            # which residues bond to a neighbour: a chain residue's topology names a +/- atom
            content.provision_residueobjects()
            for resname, resi in content.residues.items():
                refs = any(n[:1] in '+-' for b in resi.bonds for n in (b.name1, b.name2))
                (linked if refs else standalone).add(resname)
                heavy_atoms.setdefault(resname, sorted(a.name for a in resi.atoms if a.element != 'H'))
        derived = derive_segtypes(merged, curated_names=curated_resname_set(),
                                  water_resnames=water_resnames(), standalone=standalone - linked)
        ...
        self._write_truncated_resname_table(heavy_atoms, out_path.parent / 'truncated_resnames.json')
        return derived

    @staticmethod
    def _write_truncated_resname_table(heavy_atoms: dict, path) -> dict:
        """
        Write the table that lets a residue name cut to four characters be restored from its atoms.

        Several writers keep only four columns of a residue name, and CHARMM's longer names collide
        when cut: BGLCNA, BGLCA and BGLCN all become BGLC, which is itself beta-glucose.  For every
        four-character stem shared by a longer CHARMM residue, this records each candidate's heavy
        atoms (the stem itself included when it is a residue), so ingest can pick by content.
        """
        import json
        stems: dict[str, dict] = {}
        for resname in heavy_atoms:
            if len(resname) > 4:
                stems.setdefault(resname[:4], {})
        for stem, cands in stems.items():
            for resname, atoms in heavy_atoms.items():
                if resname == stem or (len(resname) > 4 and resname[:4] == stem):
                    cands[resname] = atoms
        table = {stem: cands for stem, cands in sorted(stems.items()) if any(len(c) > 4 for c in cands)}
        doc = {'_comment': 'GENERATED by pestifer modify-package charmmff regenerate-segtypes; do not edit by hand. '
                           'stem -> {CHARMM residue: heavy atom names} for residue names cut to four characters.',
               'stems': table}
        with open(path, 'w') as f:
            json.dump(doc, f, indent=0, sort_keys=True)
        logger.info(f'Wrote {len(table)} truncated-resname stems to {path}')
        return table

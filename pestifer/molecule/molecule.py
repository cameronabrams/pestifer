...
class Molecule:
    ...
    def protein_loop_gaps(self, min_length=None):
        ...
        Returns one dict per built loop copy (each biological-assembly transform of each
        asymmetric-unit gap): ``{'segname', 'loop_resids': [int, ...], 'n_anchor_resid',
        'c_anchor_resid'}`` where ``segname`` is the transform-mapped (actual) segid,
        ``loop_resids`` are the model-built loop residues (N->C), ``n_anchor_resid`` is the
        last resolved residue before the loop, and ``c_anchor_resid`` is the first resolved
        residue after it (the closure target). Uses the same interior-loop gating as
        :meth:`write_gaps`.

        Every copy is listed independently: the closer builds each loop against the actual
        surrounding structure (including any copies already closed), so the copies may adopt
        different conformations. Symmetry of the modeled-in residues is not preserved -- which
        is deliberate: it lets copies converging at an assembly axis interleave and avoid one
        another rather than being forced into an identical, mutually clashing conformation.
        """
        if min_length is None:
            min_length = getattr(self, 'min_loop_length', 4)
        ba = self.active_biological_assembly

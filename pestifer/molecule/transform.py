...
class Transform(BaseObj):
    ...
    def includes(self, *labels) -> bool:
        """
        Report whether this image is built from the named asymmetric-unit chain or segment.

        ``chainIDmap`` is generated by :meth:`generate_chainIDmap` from ``applies_chainIDs``
        (plus resolved daughters and structured glycan segnames), so when it is non-empty it
        names *exactly* the A.U. chains this image consists of -- it is authoritative, not
        advisory.  A biological assembly that selects a subset of the A.U. therefore omits the
        unselected chains from every transform's map, and a consumer that falls back to the
        A.U. name for an unmapped label (``chainIDmap.get(x, x)``) silently builds material the
        assembly excludes.

        An *empty* map carries no information -- no assembly is active, or the map has not been
        generated yet -- so every label is taken to belong.

        Parameters
        ----------
        *labels : str
            Candidate A.U. labels for one piece of structure, most specific first, e.g. a
            segment's ``segname`` and its ``chainID``.  Glycan segments are mapped under their
            structured segname (``AG01``) while their ``chainID`` is the parent protein chain,
            and either naming is sufficient.

        Returns
        -------
        bool
            True if this image contains the named structure.
        """
        chainIDmap = self.chainIDmap or {}
        if not chainIDmap:
            return True
        return any(label in chainIDmap for label in labels if label)

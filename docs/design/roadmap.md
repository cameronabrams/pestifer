...
- [x] **GPU mode detection shouldn't hinge on path inequality.** `namd.processor-type` is now a
      real `cpu | gpu | auto` control (default `auto` = the previous path-inequality inference,
      preserving behavior). `gpu` forces GPU-resident mode — including the single-binary case the
      inference could never express — and `cpu` forces CPU mode; both override the paths.
      Forcing `gpu` while `paths.namd3gpu` still points at the CPU binary is caught with an
      actionable warning (`Config._verify_gpu_electable`), and the unused-GPU warning for the
      `auto` trap (`Config._warn_gpu_not_elected`, c1837aba) remains. Executable selection is
      still driven purely by `paths.namd3gpu`/`paths.namd3`.

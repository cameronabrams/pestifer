...
``<chain1>`` and ``<chain2>`` are the chain IDs, ``<resi1>`` and ``<resi2>`` are the residue numbers of the two residues that form the link, and ``<atom1>`` and ``<atom2>`` are the atom names of the two atoms that form the link.  For example, ``A_12_O-C_15_N`` would create a covalent bond between the oxygen atom of residue 12 in chain A and the nitrogen atom of residue 15 in chain C.  You can use either 1-byte or 3-byte residue names.

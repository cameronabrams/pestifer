...
The pdb files ``DOPC-00.pdb`` through ``DOPC-09.pdb`` are the 10 different configurations of the DOPC molecule.  The ``DOPC-init.pdb`` and ``DOPC-init.psf`` files are the initial coordinates and topology of the molecule, and the ``init.tcl`` file is a psfgen script used to generate those two files.  The ``info.yaml`` file contains metadata about the RESI, such as its long name and measurements of its dimensions and reference atoms that the grid packer needs:
...
.. _subs_make_pdb_collection_sampler:

Choosing a conformer sampler
++++++++++++++++++++++++++++

``--sampler`` selects how the conformers in a collection are generated:

``md`` (the default)
  Short vacuum MD sampling.  This is the legacy sampler.  Because a lipid in vacuum has nothing to melt its tails against, the conformers it yields are extended, rod-like chains.

``mc``
  An **athermal Monte Carlo** sampler that melts the acyl chains inside a confinement cylinder, yielding fluid-like conformers whose tail disorder resembles a real bilayer.

The distinction matters for membrane packing.  A leaflet gridded from extended rods packs like a gel phase and resists relaxing to a realistic area per lipid; conformers from the ``mc`` sampler reach fluid areas per lipid readily.  **The lipid collection that ships with pestifer is built with the** ``mc`` **sampler**, so you only need this flag when generating your own collection and you want it to match.

.. code-block:: bash

    $ pestifer make-pdb-collection --streamID lipid --substreamID yeast \
        --output-dir lipid-yeast --sampler mc

Two options tune the ``mc`` sampler's confinement cylinder, which is what sets how tightly the chains are packed while they melt:

  - ``--cylinder-apl`` is the cylinder's area per lipid in Å².  Left unset, it is **auto-sized from the lipid's acyl-chain count** (``n_chains`` × 30 Å²), so a four-chain cardiolipin is not crushed into a cylinder sized for a two-chain lipid.
  - ``--cylinder-inflation`` (default 1.9) geometrically inflates the cylinder cross-section over ``--cylinder-apl``.

Sterols are a special case under either sampler: they are rigid fused-ring systems with nothing meaningful to sample, so they are always written as a **single** conformer.

.. note::

   Pestifer's lipid collection also ships ordered (**Lo**) conformer ensembles for the lipids that can form a liquid-ordered phase, stored alongside the fluid ones under keys of the form ``<RESNAME>__Lo``.  These are generated by the same ``mc`` sampler with a torsional trans-bias tuned to a target chain-order parameter.  You select them per leaflet in a build with ``composition.upper_leaflet_phase`` / ``lower_leaflet_phase``; see :ref:`make_membrane_system <subs_buildtasks_make_membrane_system>`.

   A composition naming a phase with no cached ensemble makes the build generate it on the spot, which is correct but happens while the build waits.  To do it ahead of time -- on a login node, say, rather than inside a job -- use :ref:`pestifer cache prebuild <subs_cache>`, which generates one ``(lipid, phase)`` entry with the sampler a build would use.  ``make-pdb-collection`` itself writes an entry directory for you to install, and knows nothing about phases.

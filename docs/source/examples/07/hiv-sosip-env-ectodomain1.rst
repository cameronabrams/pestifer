...
The relevant biological assembly is a C3-symmetric homotrimer of protomers, which is labeled biological assembly 1 in the PDB header.  Here we specify that the new chains generated by BIOMT transforms are H and J for chain G and C and D for chain B.  Pestifer models in unresolved and zero-occupancy residues by default.  Undoing the engineered mutations is *not* automatic, however: the three SOSIP stabilizing changes in 4zmj -- the ``SOS`` disulfide (A501C and T605C) and the ``IP`` substitution (I559P) -- are reverted to wild-type because the ``mutations`` block in the configuration below asks for it.  Omit that block and you build the stabilized construct instead.  A build of the 4zmj trimer illustrates these capabilities.

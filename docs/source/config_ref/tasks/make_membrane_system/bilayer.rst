...
Single-valued attributes:
  ...
  * ``packer``: How to place the lipids (and chamber solvent). 'grid' (the only supported packer) places lipids on a per-leaflet 2D lattice (oriented, tails toward the midplane) and solvent on a 3D lattice, relying on the relaxation MD to resolve the initial overlaps. (The former 'packmol' packer has been removed.) (default: grid)

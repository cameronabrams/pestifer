...
- new feature: `ring_check` can now check **protein side-chain rings** (His/Phe/Tyr/Trp/Pro) for piercing and resolve them by rotating the side chain rather than by deletion. Opt in by adding `protein` to `ring_check.segtypes`. Cycle search is length-bounded (`max_ring_size`, default 7) so the protein backbone closed by a disulfide -- a giant chordless cycle -- is skipped while real 5/6-membered rings are found. A pierced protein ring is fixed by trying chi2 (then chi1) rotations of the offending side chain (via the `PestiferCRot` `SCrot_chi2`/`SCrot_chi1` procs, whose `SCrot_chi2` also had a coordinate-parsing bug fixed here), re-checking after each candidate, and keeping the first rotation that clears it; if none clears it, the build stops with the residue named and the dihedral-rotation fix suggested
